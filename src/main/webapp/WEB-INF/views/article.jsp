    ...
    <body class="sb-nav-fixed">
		...
		<div class="container" role="main">
			...
			<div class="bg-white rounded shadow-sm">
				...
				<div class="board_title">${readArticle.title}</div>
				<div class="board_info_box">
					<span class="board_author">ID : ${readArticle.nickName}</span>
					
				</div>
	<%-- <c:out value="${readArticle.content}"/> --%>
				<div class="board_content">${readArticle.content}</div>
				<div class="board_tag">Area : ${readArticle.area}</div>
			</div>
			
			<div style="margin-top : 20px">
			<form name="readForm" role="form" method="post" action="delete" id='frm'>
					<input type="hidden" id="bno" name="bno" value="${readArticle.bno}" />
				<c:if test="${member.nickName == readArticle.nickName }">
				<button type="button" class="btn btn-sm btn-primary" id="btnUpdate" onclick="submit2()">수정</button>
					<script>
						function submit2(){
							var frm = document.getElementById('frm');
							frm.action='updateView';
							frm.submit();
				...
				<button type="button" class="btn btn-sm btn-primary" id="btnList" onclick="location.href='tables'">목록</button>
				</form>
			</div>
		</div>
	</article>
        ...
        <link rel="stylesheet" href="assets/dist/css/article.css" />
    </body>
</html>
